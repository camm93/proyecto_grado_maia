...
Uso T2 (binaria):
    ...
    predictor = get_predictor("scibert-es-t2")
...
Uso T1 (multiclase, devuelve todas las probabilidades):
    ...
    clf = get_t1_classifier("scibert-es")
...
def _verify_weights(model_path: Path) -> None:
    """Verifica que el directorio tenga config.json y pesos."""
    if not model_path.exists():
        raise ModelNotAvailableError(
            f"Modelo no encontrado en {model_path}. "
            f"Descarga los pesos y colócalos allí."
        )
    if not (model_path / "config.json").exists():
        raise ModelNotAvailableError(
            f"Falta config.json en {model_path}."
        )
    weights = list(model_path.glob("*.bin")) + list(model_path.glob("*.safetensors"))
    if not weights:
        raise ModelNotAvailableError(
            f"No se encontraron pesos en {model_path}. "
            "El directorio existe pero está vacío o incompleto."
        )
...
class T1Classifier:
    ...
    def __init__(self, model_id: str, device: str | None = None):
        ...
        # Resolución del device
        if device is None:
            device = os.getenv("T1_DEVICE", "auto").lower().strip()
        if device == "auto":
            device = "cuda" if torch.cuda.is_available() else "cpu"
        if device == "cuda" and not torch.cuda.is_available():
            log.warning("T1_DEVICE=cuda solicitado pero CUDA no disponible. Usando CPU.")
            device = "cpu"
        self.device = device

        log.info(f"Cargando modelo T1 '{model_id}' desde {model_path} (device={device})...")
        self.tokenizer = AutoTokenizer.from_pretrained(str(model_path))
        self.model = AutoModelForSequenceClassification.from_pretrained(str(model_path))
        self.model.eval()
        self.model.to(device)

        # Half precision en GPU acelera ~1.5x sin pérdida apreciable de precisión
        if device == "cuda":
            try:
                self.model = self.model.half()
                self._use_fp16 = True
                log.info(f"Modelo T1 '{model_id}' cargado en GPU con FP16")
            except Exception as e:
                log.warning(f"No se pudo convertir a FP16: {e}. Usando FP32.")
                self._use_fp16 = False
        else:
            self._use_fp16 = False
            log.info(f"Modelo T1 '{model_id}' cargado en {device} (FP32)")
    ...
    def warmup(self) -> None:
        ...
        Recomendado llamarlo al startup del servidor: la primera predicción real
        del usuario no paga el costo de inicialización.
        """
        try:
            log.info(f"Warming up T1 '{self.model_id}'...")
...
def list_available_models() -> list[str]:
    """Lista los modelos con pesos descargados en MODELS_DIR."""
    if not MODELS_DIR.exists():
        return []
    return [
        d.name for d in MODELS_DIR.iterdir()
        if d.is_dir() and (
            list(d.glob("*.bin")) or list(d.glob("*.safetensors"))
        )
    ]
